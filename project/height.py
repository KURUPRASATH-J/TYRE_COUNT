...
output_data = {
    'measurements': height_data,
    'statistics': {
        'frames_processed': int(frames_processed),
        'both_detections': int(both_det_count),
        'quality_rejected': int(quality_rejected),
        'valid_measurements': len(heights),
        'success_rate': float(len(heights)/both_det_count*100) if both_det_count > 0 else 0.0
    }
}
...
if heights:
    output_data['statistics'].update({
        ...
        'height_range': float(max(heights) - min(heights))
    })
    
    conf1_values = [d['conf1'] for d in height_data]
    conf2_values = [d['conf2'] for d in height_data]
    output_data['statistics'].update({
        'cam1_conf_mean': float(np.mean(conf1_values)),
        'cam1_conf_std': float(np.std(conf1_values)),
        'cam2_conf_mean': float(np.mean(conf2_values)),
        'cam2_conf_std': float(np.std(conf2_values))
    })

with open('measurement_data.json', 'w') as f:
    json.dump(output_data, f, indent=2)

print(f"\n✓ Measurement data saved to: measurement_data.json")
